...
AhoCorasickAutomaton::AhoCorasickAutomaton() {
  ...
  for (uint32_t i = 0; i < ac_base_.size(); ++i) { ac_free_list_.insert(i); }
...
bool AhoCorasickAutomaton::IsAcUsed(uint32_t pos) const { return ac_free_list_.find(pos) == ac_free_list_.end(); }
...
void AhoCorasickAutomaton::AcInsert(uint32_t father_pos, const std::vector<char32_t> &children_chars_to_insert) {
  ...
  all_children_chars.erase(std::unique(all_children_chars.begin(), all_children_chars.end()), all_children_chars.end());
  ...
  std::cerr << "DEBUG: AcInsert father_pos = " << father_pos << ", old base = " << old_add << ", new base = " << new_add
            << "\n";// Debug output
      ...
      if (old_child_pos != new_child_pos) {
        ...
        for (auto const &[grandchild_char_val, grandchild_old_pos]: ac_node_children_map_[old_child_pos]) {
          if (grandchild_old_pos < static_cast<int>(ac_check_.size()) &&
            ...
            std::cerr << "DEBUG:     Updated grandchild " << grandchild_old_pos << " check from " << old_child_pos
                      << " to " << new_child_pos << "\n";// Debug output
          }
        }
...
void AhoCorasickAutomaton::add_pattern(const std::string &pattern_str, int pattern_id) {
  ...
  for (size_t i = 0; i < pattern_sent.size(); ++i) {
    ...
    if (target_idx_candidate >= static_cast<int>(ac_check_.size()) ||
      ...
      children_for_insert.erase(
          std::unique(children_for_insert.begin(), children_for_insert.end()), children_for_insert.end()
      );
    ...
    while (temp_state != 0) {
      ...
      if (!output_patterns_[temp_state].empty()) {
        for (int pattern_id: output_patterns_[temp_state]) {
          ...
          std::cout << "  模式 '" << converter.to_bytes(pattern_id_to_string_[pattern_id]) << "' (ID: " << pattern_id
                    << ") 在索引 [" << start_index << ", " << i << "] 处找到。\n";
...
void AhoCorasickAutomaton::print_ac_trie_info() {
  ...
  for (int i = 0; i < print_limit; ++i) {
    ...
    for (int pid: output_patterns_[i]) { std::cerr << pid << " "; }
