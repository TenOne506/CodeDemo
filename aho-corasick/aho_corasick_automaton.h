...
public:
  ...
  std::vector<std::map<char32_t, int>>
      ac_node_children_map_; // Child mapping for the internal Trie, key is
                             // char32_t

  // Aho-Corasick specific members
  std::vector<int> fail_link_;
  std::vector<std::vector<int>>
      output_patterns_; // Stores IDs of patterns ending at this node
  std::vector<int> dictionary_match_link_; // Link for output propagation

  std::map<int, int>
      pattern_id_to_length_; // Map from pattern ID to pattern length
  std::map<int, std::u32string>
      pattern_id_to_string_; // Map from pattern ID to pattern string
                             // (u32string)
...
private:
  ...
  void AcInsert(uint32_t father_pos,
                const std::vector<char32_t> &children_chars_to_insert);
