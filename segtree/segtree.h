...
class SegmentTree {
    ...
    void maintain(int node) { tree[node] = merge_val(tree[node * 2], tree[node * 2 + 1]); }
 ...
 public:
    ...
    // 线段树维护数组 a
    explicit SegmentTree(const vector<T>& a) : n(a.size()), tree(2 << bit_width(a.size() - 1)) { build(a, 1, 0, n - 1); }
    ...
    // 时间复杂度 O(log n)
    T query(int ql, int qr) const { return query(1, 0, n - 1, ql, qr); }

    // 获取 a[i] 的值
    // 时间复杂度 O(log n)
    T get(int i) const { return query(1, 0, n - 1, i, i); }
};
